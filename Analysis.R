...
#' @:Ke_Zhang ---- 2026.02.25
#' 
#' This script is mainly for investigating the associations between aging-related multimorbidity trajectories and personalized glycemic patterns. 
#' It is also applied for screening multimorbidity-related proteomic signatures.
#' 
#' @:-----------------------------------------------------------------------------------------
...
#' @:Pre-setting
rm(list = ls())  
root <- "[Your root pathname]"
workpath <- paste(root, "[Your work folder name]", sep="/")


#' @:Loading 
#' Read in packages and predifined functions
list <- list.files(path=paste0(root,"/[Folder name]/function"), full.names = TRUE)
lapply(list, source) 
...
#' @:-----------------------------------------------------------------------------------------
#' 
#' @:Part0: Aging-related multimorbidity trajectories linked to divergent glycemic patterns
#' 
#' @:-----------------------------------------------------------------------------------------
...
#' @:section1: Aging-&-DailyTraits [CV, Slicing window]
##-----------------------------------------------------------
# data ---------------------------------------  
...
#' @:section2: Aging-&-DRs [CV, Slicing window]
##-----------------------------------------------------------
# data --------------------------------------- 
#' @:GNHS
setwd(paste0(root,"/1_data"))
...
#' @:section3: Aging-&-PGS [CV, Slicing window]
##-----------------------------------------------------------
# data --------------------------------------- 
#' @:GNHS
setwd(paste0(workpath, "/part1_aging"))
...
#' @:section1: Uniqueness-Aging [Spearman correlation] 
##-----------------------------------------------------------
# data --------------------
...
#' @:section2: Uniqueness-Disease [Correlation between disease trajectory and dietary patterns]  
##-----------------------------------------------------------
# data --------------------------------------- 
setwd(paste0(workpath, "/part1_aging"))
...
result2 <- stat_pSC(data = database, list1 = c("unique_disease","PGS",list2), list2 = c("sex","age"), method = "spearman") 
result2 <- result2[result2$feat1=="unique_disease",]
...
#' @:-----------------------------------------------------------------------------------------
#' 
#' @:Part1: Disease duration of morbidities and CGM-derived daily glycemic traits
#' 
#' @:-----------------------------------------------------------------------------------------
...
#' @:LADE-daily [Daily glycemic traits, all samples & non-diabetic, *** updated]
##------------------------------------------------------
# data ------------------------------  
setwd('/Volumes/zhangke/Projects/1_Cohort_Database/GNHS_Guangzhou Nutrition and Health Study/phenotype')
data_diet.CHEI <- openxlsx::read.xlsx("GNHS_diet_DietaryPattern_all_250811.xlsx", sheet = "all")
data_diet.CHEI$visit <- as.numeric(gsub("F","",data_diet.CHEI$visit))
data_diet.CHEI <- data_diet.CHEI[order(data_diet.CHEI$visit, decreasing = T),]
data_diet.CHEI <- data_diet.CHEI[!duplicated(data_diet.CHEI$id),]; table(data_diet.CHEI$visit)
data_diet.CHEI$sampleid <- paste0("F4",data_diet.CHEI$id)
...
database <- merge(data_pheno.GNHS[,c("sampleid","age","sex","BMI","income3","education3","smoke_whe","MET","Diabetes_med_whe")], data_CGM[,c("id",list_daily)], by.x = "sampleid", by.y = "id", all.y=T)
database <- merge(data_diet.CHEI[,c("sampleid","score_chei")], database, by.x = "sampleid", by.y = "sampleid", all.y=T)
...
list_covar2 <- c("age","sex","BMI","income3","education3","smoke_whe","MET","score_chei")
...
for(time in list){
  ...
  result1 <- stat_linear(data = input_data, list_exp = setdiff(list_exp,"otc_ObesOverwe"), list_otc = list_otc2, list_covar = list_covar2)  
  result2 <- stat_linear(data = input_data, list_exp = "otc_ObesOverwe", list_otc = list_otc2, list_covar = setdiff(list_covar2,"BMI")) 
  result <- rbind.fill(result1, result2)
...
out_linear.mod2 <- out_linear

out_linear.all <- rbind(data.frame(mod="mod1", out_linear.mod1), data.frame(mod="mod2", out_linear.mod2))
...
out_linear.noT2D.mod1 <- out_linear
...
out_linear.noT2D.mod2 <- out_linear
out_linear.noT2D <- rbind(data.frame(mod="mod1", out_linear.noT2D.mod1), data.frame(mod="mod2", out_linear.noT2D.mod2))

a <- merge(out_linear.mod1, out_linear.mod2, by=c("outcome","exposure"), all=F)
plot(a$beta.x, a$beta.y)
b <- merge(out_linear.noT2D.mod1, out_linear.noT2D.mod2, by=c("outcome","exposure"), all=F)
plot(b$beta.x, b$beta.y)


##------------------------------------------------------
setwd(paste0(workpath, "/part1_disease"))
openxlsx::write.xlsx(list("stat_all"=out_linear.mod1, "sensi_all"=out_linear.mod2, "stat_noT2D"=out_linear.noT2D.mod1, "sensi_noT2D"=out_linear.noT2D.mod2, "data"=database), "stat_Daily_LADE_linear.xlsx")




#' Implementation of Bayesian Network (BN) structure learning and causal inference to map the complex interdependencies among various diseases.
...
#' investigation of the difference in longitudinal trajectories of multimorbidity stratified by baseline dyslipidemia status.
...
#' @:-----------------------------------------------------------------------------------------
#' 
#' @:Part2: Systemic multimorbidity index [MMI-system] and daily glycemic traits
#' 
#' @:-----------------------------------------------------------------------------------------
...
#' Comparison of glycemic variability across different levels of multimorbidity severity by using multi-variable linear regression analysis.
...
database <- merge(database, data[,c("id","income3","education3","smoke_whe","MET","score_chei")], by="id", all = F) 
...
output_linear2.mod1 <- output_linear2


# stat (sensi, model2) ----------------------
list_covar2 <- c("age","sex","BMI","income3","education3","smoke_whe","MET","score_chei")

result_all <- stat_linear_factor(data = database, list_exp = "group", list_otc = list_daily, list_covar = list_covar2) 
result_noT2D <- stat_linear_factor(data = database[database$otc_T2D==0,], list_exp = "group", list_otc = list_daily, list_covar = list_covar2) 
result_T2D <- stat_linear_factor(data = database[database$otc_T2D==1,], list_exp = "group", list_otc = list_daily, list_covar = list_covar2) 
...
table(output_linear2$group2, output_linear2$sig_q)
table(output_linear2$time, output_linear2$group2, output_linear2$sig_q)
output_linear2.mod2 <- output_linear2

plot(output_linear2.mod1$beta, output_linear2.mod2$beta)
rcorr(output_linear2.mod1$beta, output_linear2.mod2$beta)


##------------------------------------------------------
setwd(paste0(workpath, "/part2_multimorbid"))
openxlsx::write.xlsx(list("stat"=output_linear2.mod1, "sensi"=output_linear2.mod2, "data"=database), "stat_multimorbidity_DailyTraits_linear_quartile.xlsx")
...
#' @:-----------------------------------------------------------------------------------------
#' 
#' @:Part3: Systemic multimorbidity index, PGS, and dietary responses
#' 
#' @:-----------------------------------------------------------------------------------------
...
#' Wilcoxon tests and linear regression to observe how PGS associates with postprandial glycemic responses to standardized meal tests.
...
database <- merge(data_CGM, data_disease[,c("id","age","sex","BMI","income3","education3","smoke_whe","MET",list_comorb,"otc_T2D")], by="id", all=F)   
print(paste0("N of sample size: ", length(unique(database$id))))
...
#' Comparison of the relationship between MMI-system and PGS stratified by multimorbidity severity.
#' @:Multimorbidity-DRs [*** updated]
##------------------------------------------------------
# data ------------------------------ 
#' @:Dietary-responses
setwd(paste0(root,"/1_data")) 
...
database <- merge(data_CGM, data_disease[,c("id","age","sex","BMI","income3","education3","smoke_whe","MET","score_chei",list_comorb,"otc_T2D")], by="id", all=F)   
print(paste0("N of sample size: ", length(unique(database$id))))
...
out_linear.all.mod1 <- out_linear.all


#' @:mod2------------------------
out_linear <- data.frame()
for(meal in list_meal){
  result <- stat_linear(data = input_data[input_data$meal==meal,], list_exp = list_exp, list_otc = list_otc, list_covar = list_covar2, timepoint = meal)
  result <- data.frame(meal=meal,result)
...
out_linear.all <- out_linear
table(out_linear.all$meal, out_linear.all$outcome, out_linear.all$sig_q)
out_linear.all.mod2 <- out_linear.all
...
out_linear.noT2D.mod1 <- out_linear.noT2D
...
out_linear.noT2D <- out_linear
table(out_linear.noT2D$meal, out_linear.noT2D$outcome, out_linear.noT2D$sig_q)
out_linear.noT2D.mod2 <- out_linear.noT2D


##------------------------------------------------------
setwd(paste0(workpath, "/part3_PGS"))
openxlsx::write.xlsx(list("stat_all"=out_linear.all.mod1, "sensi_all"=out_linear.all.mod2, 
                          "stat_noT2D"=out_linear.noT2D.mod1, "sensi_noT2D"=out_linear.noT2D.mod2, "data"=database), "stat_DRs_multimorbidity_linear.xlsx")




#' Regression of the relationship between MMI-system and PGS stratified by multimorbidity severity.
#' @:PGS-multimorbidity
#' @:linear [Cross-sectional, one point, *** updated] 
##------------------------------------------------------
# data ------------------------------ 
setwd(paste0(root, "/1_data")) 
...
database <- merge(database, data[,c("id","income3","education3","smoke_whe","MET","score_chei")], by="id", all.x = T)
print(paste0("N of sample size: ", length(unique(database$id))))
...
output_tertile.mod1 <- rbind(data.frame(group="all", out_linear.all), data.frame(group="noT2D", out_linear.noT2D))


#' @:mod2
input_data <- database 
input_data$PGS <- scale(input_data$PGS)
out_linear.all <- stat_linear_factor(data = input_data, list_exp = list_exp, list_otc = list_otc, list_covar = list_covar2)

input_data <- database[database$otc_T2D==0,]
input_data$PGS <- scale(input_data$PGS)
list_exp <- setdiff(list_exp,"otc_T2D") 
out_linear.noT2D <- stat_linear_factor(data = input_data, list_exp = list_exp, list_otc = list_otc, list_covar = list_covar2)

output_tertile.mod2 <- rbind(data.frame(group="all", out_linear.all), data.frame(group="noT2D", out_linear.noT2D))



##------------------------------------------------------
setwd(paste0(workpath, "/part3_PGS"))
openxlsx::write.xlsx(list("stat_conti"=output_conti, "stat_ter"=output_tertile.mod1, "sensi_ter"=output_tertile.mod2, "data"=database), "stat_PGS_multimorbidity_linear.xlsx")




#' Regression of the relationship between each disease duration and PGS.
#' @:PGS-LADE [8 glucose-related diseases, *** updated] 
#' @:linear
##------------------------------------------------------
# data ------------------------------ 
setwd(paste0(root, "/1_data")) 
...
#' @:mod2
result1 <- stat_linear(data = input_data, list_exp =setdiff (list_exp,"otc_ObesOverwe"), list_otc = list_otc, list_covar = list_covar2)
result2 <- stat_linear(data = input_data, list_exp ="otc_ObesOverwe", list_otc = list_otc, list_covar = setdiff(list_covar2,"BMI"))
out_linear.all <- rbind(result1, result2)
out_linear.all <- data.frame(out_linear.all, qval=NA, sig_q=NA)
for(i in list_otc){
  out_linear.all$qval[out_linear.all$outcome==i] <- p.adjust(out_linear.all$pval[out_linear.all$outcome==i], method = "BH")
  out_linear.all$sig_q <- ifelse(out_linear.all$pval<0.05 & out_linear.all$qval<0.05,"**",NA)
...
out_linear.T2D.mod1 <- out_linear.T2D
...
result1 <- stat_linear(data = input_data, list_exp =setdiff (list_exp,"otc_ObesOverwe"), list_otc = list_otc, list_covar = list_covar2)
result2 <- stat_linear(data = input_data, list_exp ="otc_ObesOverwe", list_otc = list_otc, list_covar = setdiff(list_covar2,"BMI"))
out_linear.T2D <- rbind(result1, result2) 
out_linear.T2D <- data.frame(out_linear.T2D, qval=NA, sig_q=NA)
for(i in list_otc){
  out_linear.T2D$qval[out_linear.T2D$outcome==i] <- p.adjust(out_linear.T2D$pval[out_linear.T2D$outcome==i], method = "BH")
  out_linear.T2D$sig_q <- ifelse(out_linear.T2D$pval<0.05 & out_linear.T2D$qval<0.05,"**",NA)
...
table(out_linear.T2D$outcome, out_linear.T2D$sig_q)
out_linear.T2D.mod2 <- out_linear.T2D


##------------------------------------------------------
setwd(paste0(workpath, "/part3_PGS"))
openxlsx::write.xlsx(list("stat_all"=out_linear.all.mod1, "sensi_all"=out_linear.all.mod2,
                          "stat_noT2D"=out_linear.T2D.mod1, "sensi_noT2D"=out_linear.T2D.mod2, "data"=database), "stat_PGS_LADE_linear.xlsx")
...
#' 
#' @:Part4
#' @:Prediction-modeling [Prediction for daily traits and postprandial glycemic responses]
#' 
#' @:-----------------------------------------------------------------------------------------
...
#' Performance evaluation: comparison of basic models versus integrated models.
#' @:Function-definition
##------------------------------------------------------
# continuous ---------------------
...
# seed list ---------------------
set.seed(123456)
list_seed <- sample(1:10000,100, replace = F)
...
list_triat <- c("ALT","AST","ALP","ratio_AST_ALT",
                "waist","hip","WHR","ratio_AG","PFAT_trunck","PFAT_total","PFAT_android","PFAT_gynoid","LAP","VAI",
                "SBP","DBP","HR",
                "TC", "TG", "HDL", "LDL","ratio_TyG","ratio_TG_HDL",
                "blood_Crea","urine_Crea"
) 
...
list_merge <- list(list_triat, list_comorb)
list_name <- c("MMI-trait","MMI-system")
...
for(s in list_seed){
  ...
  for(y in list_y){ 
    ...
    formula <- paste0(y," ~ ", a)
    result <- model_eval_conti(model_rf_conti(submatr, trait = y, list_x = list_x, formula=formula, nfold = 10, seed = s)) 
    result <- data.frame(seed=s, outcome=y, predictor="T2D", result)
    output_perf <- rbind.fill(output_perf, result)
...
openxlsx::write.xlsx(list("perf_base"=output_perf.base, "perf_comb"=output_perf.comb, "data"=database), "stat_Comorbidity_Daily_predict_all.xlsx")
...
#' @:-----------------------------------------------------------------------------------------
#' 
#' @:Part5: Systemic multimorbidity index and serum proteomics
#' 
#' @:-----------------------------------------------------------------------------------------
...
#' Functional enrichment: GO enrichment analysis for biological pathways associated with metabolic comorbidity.
...
#' Regression associations between selected proteins and CGM-daily traits & meal traits
#' @:Section1: [Protein-Dailys]
##----------------------------------------------------------- 
# data --------------------------
setwd(paste0(root, "/1_data")) 
code_prot <- openxlsx::read.xlsx("GNHS_omics_proteomics.xlsx", sheet = "code")
...
#' Cross-organ enrichment analysis to be compared between MMI-related and other proteins.
...
database$PGS <- as.numeric(database$PGS)
...
#' PGS prediction and SHAP integration: using SHAP values to interpret the contribution of individual proteins to the prediction of PGS.
